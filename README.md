...
# 4. Point Drift Graph at your notes and run the pipeline
python -m driftgraph.build --notes ./data/notes

# 5. Serve the web app (frontend is served at http://localhost:8000/)
...
```bash
python -m driftgraph.main query "Who is Alice Chen?" --mode local
python -m driftgraph.main query "What are the main themes?" --mode global
```
